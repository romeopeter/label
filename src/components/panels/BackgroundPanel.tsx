...
import { PanelTitle, SectionLabel, LabeledSlider, ColorRow } from "./primitives";

const SWATCHES = [
  "#6E5BFF", "#A06FFF", "#3FCBA6", "#1D9E75", "#EF9F27", "#FF7A59",
  "#0F0E1A", "#1A1826", "#3C3489", "#534AB7", "#AFA9EC", "#EEEDFE",
];
      ...
      {bg.mode === "gradient" && (
        <>
          ...
          <div
            ...
            style={{ background: `linear-gradient(90deg, ${bg.gradient.from}, ${bg.gradient.to})` }}
          />
          <ColorRow
          ...
          />
          <LabeledSlider
            label="ANGLE" unit="°" min={0} max={360}
      ...
      {bg.mode === "image" && (
        <>
          ...
          {bg.imageSrc && (
            <>
              <LabeledSlider
                label="BLUR" unit="px" max={40}
                value={bg.imageBlur}
                onChange={(v) => setBg({ imageBlur: v })}
              />
              <LabeledSlider
                label="OPACITY" unit="%" max={100}
      ...
      <div className="grid grid-cols-6 gap-1.5">
        {GRADIENTS.map((g, i) => (
          <button
            ...
            onClick={() => setBg({ mode: "gradient", gradient: { ...g, angle: 135 } })}
            className="aspect-square cursor-pointer rounded-md border border-hairline transition-transform hover:scale-110 hover:border-p-300"
